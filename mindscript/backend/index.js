...
app.post("/chat-stream", async (req, res) => {
  ...
  try {
    const responseStream = await ai.models.generateContentStream({
      ...
      contents: message,
      config: {
        systemInstruction: "I want you give me answer in 50 words"
      }
    });
    ...
    console.error("Gemini Stream API error:", e?.message || e);
    res.write("\n[ERROR]");
    res.end();
  }
});
...
app.delete('/deletehistory/:id', async (req, res) => {
  ...
  try {
    const deletedItem = await HistoryModel.findOneAndDelete({ _id: id, userEmail: email });
    if (!deletedItem) {
