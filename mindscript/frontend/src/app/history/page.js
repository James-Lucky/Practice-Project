...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:14000";
...
const HistoryPage = () => {
 ...
 const [showDeleteModal, setShowDeleteModal] = useState(null)
  ...
  useEffect(()=>{
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/myhistory/${email}`)
  ...
  const handleDelete = async (itemId, index) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/deletehistory/${itemId}`, {
        method: 'DELETE',
        headers: {
    ...
    <div className="min-h-screen bg-black p-4">
      <div className="max-w-4xl mx-auto py-8">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-bold text-white mb-2 bg-gradient-to-r from-white to-purple-300 bg-clip-text text-transparent">
            Your History
          </h1>
              ...
              <div key={item.id || index} className="relative bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 shadow-xl overflow-hidden group hover:bg-white/10 transition-all duration-300">
                ...
                <div className="absolute bottom-4 right-4 flex space-x-2">
                  ...
                  <button
                    ...
                    className="group/btn bg-purple-600 hover:bg-purple-500 text-white p-3 rounded-full shadow-lg transition-all duration-300 hover:scale-110 hover:shadow-purple-500/25"
                ...
                <div className="absolute inset-0 bg-gradient-to-r from-purple-600/10 to-purple-900/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
              </div>
            ))}
          </div>
        ) : (
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden">
          ...
          <div className="absolute top-1/2 left-3/4 w-3 h-3 bg-purple-600 rounded-full animate-bounce opacity-20"></div>
          <div className="absolute top-1/6 right-1/6 w-1.5 h-1.5 bg-purple-300 rounded-full animate-pulse opacity-25"></div>
          <div className="absolute bottom-1/4 left-1/2 w-2.5 h-2.5 bg-purple-500 rounded-full animate-ping opacity-15"></div>
        </div>
      </div>
    </div>
  )
